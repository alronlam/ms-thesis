...
class UserVerticesSAWeightModifier(EdgeWeightModifierBase):
    ...
    def modify_edge_weights(self, graph, params, verbose):
        ...
        for index, tweet in enumerate(tweets):
            ...
            if verbose:
                print("SAWeightModifier: Processed {}/{} tweets".format(index+1, len(tweets)))
...
def modify_edge_weights(graph, edge_weight_modifiers, params, verbose=False):
    for modifier in edge_weight_modifiers:
        graph = modifier.modify_edge_weights(graph, params, verbose)
    return graph