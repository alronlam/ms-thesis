...
from pcari.WordEmbeddingGeneratorDriver import create_label_encoder
...
def generate_vocab_and_embedding_matrix(arguments):
    corpus = arguments.get("corpus")

    logger.info("Fetching vocab..")
    vocab = get_or_build(arguments.get("vocab_path"), build_vocab, corpus)
    logger.info("Vocab has %i elements.\n", len(vocab))

    logger.info("Fetching cooccurrence list..")
    # corpus.seek(0)
    cooccurrences = get_or_build(arguments.get("cooccur_path"),
    ...
    if arguments.get("save_often"):
        iter_callback = partial(save_model, path=arguments.get("vector_path"))
    else:
        iter_callback = None
    ...
    W = train_glove(vocab, cooccurrences,
                    ...
                    vector_size=arguments.get("vector_size"),
    ...
    # save_model(W, arguments.get("vector_path"))
...
(x,y) = load_csv_dataset("data/yolanda_nov2013_feb2014_dataset_no_others.csv")
x = preprocess_tweets(x, preprocessors)


arguments = {}
arguments["corpus"] = x
arguments["window_size"] = 5
arguments["vector_size"] = 200
arguments["iterations"] = 25
arguments["learning_rate"] = 0.05
arguments["vector_path"] = "new_glove_embeddings"
(vocab, W) = generate_vocab_and_embedding_matrix(arguments)

def convert_to_word_sequence(vocab, text):
    word_sequence_arr = []
    text_arr = text.split()
    for word in text_arr:
        (index, frequency) = vocab[word]
        word_sequence_arr.append(index)
    return word_sequence_arr

def convert_to_word_sequences(x, vocab):
    return [convert_to_word_sequence(vocab,entry) for entry in x]


x = convert_to_word_sequences(x, vocab)
...
np.savez(arguments["vector_path"],
                x=x, y=y,
                embedding_matrix=W)
